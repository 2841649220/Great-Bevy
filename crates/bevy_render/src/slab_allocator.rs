    ...
    pub fn copy_element_data(
        ...
        match *slab {
            ...
            Slab::LargeObject(ref mut large_object_slab) => {
                ...
                // M1-4b-2: the mapped-at-creation write path is replaced by a
                // CPU-side staging fill + `create_buffer_with_data` (the
                // diligent path has no mapped-at-creation buffers).
                let mut data = alloc::vec![0u8; len];
                fill_data(WriteOnly::from_mut(&mut data[..]));
