...
impl<T: ShaderType + WriteInto> DynamicUniformBuffer<T> {
    ...
    pub fn get_writer<'a>(
        ...
        let mut capacity = self.buffer.as_ref().map(|b| b.size()).unwrap_or(0);
        let size = alignment
...
/// A wrapper to work around the orphan rule so that [`QueueWriteBufferView`] can implement
/// [`BufferMut`].
struct QueueWriteBufferViewWrapper {
    buffer_view: QueueWriteBufferView,
