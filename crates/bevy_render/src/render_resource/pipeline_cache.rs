...
use crate::{
    ...
    renderer::{
        diligent_pso,
        diligent_pso::{PipelineLayoutRecord, ShaderModuleRecord},
...
use crate::renderer::diligent_pso::DiligentBackend;
...
/// shaders must not share a record.
#[derive(Default)]
struct LayoutCache {
    layouts: HashMap<LayoutCacheKey, Arc<PipelineLayoutRecord>>,
}
...
impl LayoutCache {
    fn get(
        ...
        bg_descriptors: &[BindGroupLayoutDescriptor],
        shader_modules: &[&Arc<ShaderModuleRecord>],
        immediate_size: u32,
    ) -> Arc<PipelineLayoutRecord> {
        let bind_group_ids = bind_group_layouts.iter().map(BindGroupLayout::id).collect();
        let mut module_keys = [0usize; 2];
        for (i, module) in shader_modules.iter().enumerate().take(2) {
            module_keys[i] = module.module.registry_key() as usize;
        }
        self.layouts
            .entry((bind_group_ids, immediate_size, module_keys))
            .or_insert_with_key(|(_, _, _)| {
                ...
                // The PSO-side PRS per bind group (shader-derived names).
                let shader_names: Vec<std::collections::HashMap<(u32, u32), String>> =
                    shader_modules
                        .iter()
                        .map(|module| {
                            module
                                .naga_module()
                                .map(diligent_pso::shader_resource_names)
                                .unwrap_or_default()
                        })
                        .collect();
                let shader_names: Vec<&std::collections::HashMap<(u32, u32), String>> =
                    shader_names.iter().collect();
                let mut prs = Vec::new();
                if let Some(device) = render_device.diligent_device() {
                    for (group, descriptor) in bg_descriptors.iter().enumerate() {
                        match diligent_pso::create_shader_named_prs(
                            device,
                            group as u32,
                            descriptor,
                            &shader_names,
                    ...
                    let record = Arc::new(PipelineLayoutRecord {
                        prs,
                        immediate_prs,
                        immediate_name,
                        immediate_probe_stages,
                        immediate_size,
                        layout,
                    });
                    render_device.register_pipeline_layout(&record);
                    record
                } else {
                    Arc::new(PipelineLayoutRecord {
                        prs,
                        immediate_prs: None,
                        immediate_name: None,
                        immediate_probe_stages: ShaderStages::empty(),
                        immediate_size,
                        layout,
                    })
                }
...
fn load_module(
...
) -> Result<ShaderModuleRecord, ShaderCacheError> {
    let shader_source = match shader_source {
        #[cfg(feature = "shader_format_spirv")]
        ShaderCacheSource::SpirV(data) => util::make_spirv(data),
    ...
    // M1-4b-2: the wgpu error-scope mechanism is gone - the shader module
    // creation compiles the source to naga and latches compilation errors,
    // which `load_module` reads back below.
    let shader_module = match validate_shader {
        ValidateShader::Enabled => {
            render_device.create_and_validate_shader_module(module_descriptor.clone())
        }
        ...
        ValidateShader::Disabled => unsafe {
            render_device.create_shader_module(module_descriptor.clone())
        },
    };

    if let Some(description) = render_device.take_shader_validation_error() {
        return Err(ShaderCacheError::CreateShaderModule(description));
    }

    Ok(ShaderModuleRecord::new(shader_module))
}
...
pub struct PipelineCache {
    ...
    shader_cache: Arc<Mutex<ShaderCache<ShaderModuleRecord, RenderDevice>>>,
    device: RenderDevice,
...
impl PipelineCache {
    ...
    fn start_create_render_pipeline(
        ...
        let bind_group_layout = descriptor
            ...
            .collect::<Vec<_>>();
        let bg_descriptors = descriptor.layout.clone();
        // M2a task 6: capture the sync/async switch for the Diligent PSO
        // creation inside the async task (the closure cannot borrow `self`).
        let self_sync = self.synchronous_pipeline_compilation;
        ...
        create_pipeline_task(
            async move {
                ...
                // TODO-REMOVE-M1-4: register the shader records so the
                // wgpu descriptors can be resolved back to the Diligent
                // objects inside create_render_pipeline.
                device.register_shader_module(&vertex_module);
                if let Some(fragment) = &fragment_module {
                    device.register_shader_module(fragment);
                }

                let layout = if descriptor.layout.is_empty() && descriptor.immediate_size == 0 {
                    None
                } else {
                    let mut modules: Vec<&Arc<ShaderModuleRecord>> = vec![&vertex_module];
                    if let Some(fragment) = &fragment_module {
                ...
                Ok(Pipeline::RenderPipeline(
                    // M2a task 6: Diligent-native async PSO compilation.
                    // V20 verified dGPU async cold-start is 1.7-3.1x faster
                    // wall-clock than sync (30-229ms vs 17-75ms). The async
                    // path submits the PSO with PSO_CREATE_FLAG_ASYNCHRONOUS
                    // and then polls GetStatus (blocking the *async-pool*
                    // worker thread, never the render thread) until READY so
                    // the promoted pipeline is always usable; engines without
                    // async support ignore the flag and return a ready PSO
                    ...
                    if device.diligent_backend() != DiligentBackend::Other && !self_sync
                    {
                        ...
                        // the future `Send` (no guard is held across an
                        // await).
                        while !pipeline.diligent_pso_ready() {
                            std::thread::sleep(std::time::Duration::from_millis(2));
                        }
                        if pipeline.diligent_pso_failed() {
                            bevy_log::warn!(
                                "diligent: render pipeline {:?} async PSO compilation failed",
                                pipeline.id()
                            );
                        }
                        pipeline
    ...
    fn start_create_compute_pipeline(
        ...
        create_pipeline_task(
            async move {
                ...
                // TODO-REMOVE-M1-4: register the shader record so the wgpu
                // descriptor can be resolved back to the Diligent object.
                device.register_shader_module(&compute_module);

                let layout = if descriptor.layout.is_empty() && descriptor.immediate_size == 0 {
                    None
                } else {
                    let modules: Vec<&Arc<ShaderModuleRecord>> = vec![&compute_module];
                    Some(layout_cache.get(
                        &device,
                        &bind_group_layout,
                        &bg_descriptors,
