...
/// Shared diligent-side state of a bind group: the first binding whose
/// variable could not be bound when the SRB was built (M1-3 review, fix 3).
///
/// A resolve miss at build time still leaves the SRB object alive, and
/// committing it would bind variables that are unbound - garbage reads or
/// engine errors. The failed binding is recorded here (shared across clones
/// of the bind group, like the SRB handle), and
/// [`TrackedRenderPass::set_bind_group`](crate::render_phase::TrackedRenderPass::set_bind_group)
/// skips the commit and poisons the pass instead. `u32::MAX` marks a clean
/// SRB (no binding is ever indexed by `u32::MAX`).
pub(crate) struct BindGroupDiligentState {
    first_failed_binding: AtomicU32,
    /// Resolved `IShaderResourceVariable` pointers of the layout's
    /// dynamic-offset buffer bindings (binding -> variable), resolved once
    /// when the SRB is built (M2a §6.1.1).
    ///
    /// The engine documents that a variable pointer never changes
    /// (`GetVariableByName` note: "the pointer never changes"), so the
    /// per-draw `SetBufferOffset` path reuses it instead of re-resolving
    /// by name: a fresh lookup probes stages that are invalid for the
    /// SRB's pipeline type (e.g. VERTEX/PIXEL on a compute signature) and
    /// makes the engine log a warning per probe
    /// (ShaderResourceBindingBase.hpp:185).
    dynamic_variables: Mutex<HashMap<u32, usize>>,
}

impl BindGroupDiligentState {
    pub(crate) fn new() -> Self {
        Self {
            first_failed_binding: AtomicU32::new(u32::MAX),
            dynamic_variables: Mutex::new(HashMap::new()),
        }
    }

    /// Records the resolved SRB variable of a dynamic-offset binding
    ...
    /// ignored - one failure already makes the SRB uncommittable).
    pub(crate) fn record_failed_binding(&self, binding: u32) {
        let _ = self.first_failed_binding.compare_exchange(
            u32::MAX,
            binding,
    ...
    /// The first binding that failed to bind, when any.
    pub(crate) fn first_failed_binding(&self) -> Option<u32> {
        match self.first_failed_binding.load(core::sync::atomic::Ordering::Relaxed) {
            u32::MAX => None,
...
/// lifetime management issues. It dereferences to the diligent-side
/// [`WgpuBindGroup`] carrier (M1-4b-2: the transition wgpu handle is gone).
///
/// Can be created via [`RenderDevice::create_bind_group`](RenderDevice::create_bind_group).
#[derive(Clone)]
pub struct BindGroup {
    pub(crate) id: BindGroupId,
    /// The handle the wrapper dereferences to (the diligent SRB carrier).
    pub(crate) inner: Arc<WgpuBindGroup>,
}
...
impl core::fmt::Debug for BindGroup {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("BindGroup")
            ...
            .field(
                "diligent_poisoned",
                &self.inner.diligent_state.first_failed_binding().is_some(),
            )
            .finish()
    }
}
...
impl BindGroup {
    ...
    /// The Diligent shader resource binding, when this instance has one
    /// (consumed by the M1-3 diligent render-pass recording).
    pub(crate) fn diligent(&self) -> Option<&diligent_rs::ShaderResourceBinding> {
        self.inner.value.as_deref()
    }

    /// The first binding whose variable could not be bound on the diligent
    /// SRB, if any (M1-3 review, fix 3): committing such an SRB leaves the
    /// variable unbound - `set_bind_group` must skip the commit and poison
    /// the pass instead.
    pub(crate) fn first_failed_diligent_binding(&self) -> Option<u32> {
        self.inner.diligent_state.first_failed_binding()
    }

    /// The ascending binding indices of the layout's dynamic-offset buffer
    /// entries (M2a, §6.1.1): the `set_bind_group(.., &[u32])` offset array
    /// maps to these SRB variables one-to-one, in binding order.
    pub(crate) fn dynamic_bindings(&self) -> &[u32] {
        &self.inner.dynamic_bindings
    }
}
...
impl<'a> From<&'a BindGroup> for Option<&'a WgpuBindGroup> {
    fn from(value: &'a BindGroup) -> Self {
        Some(value.deref())
    }
}

impl<'a> From<&'a mut BindGroup> for Option<&'a WgpuBindGroup> {
