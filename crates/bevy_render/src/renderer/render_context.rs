...
struct RenderContextStateInner {
    /// The shared command encoder (records on the diligent immediate
    /// context; M1-4b-2: the wgpu transition encoder is gone).
    command_encoder: Option<CommandEncoder>,
    command_buffers: Vec<CommandBuffer>,
    render_device: Option<RenderDevice>,
    /// The Diligent immediate device context - always present when the
    /// diligent device was initialized (M1-3: the render-execution path
    /// records here; "lazy creation" degenerates to a no-op because the
    /// immediate context always exists).
...
impl<'w, 's> RenderContext<'w, 's> {
    fn ensure_device(&mut self) {
        ...
        if self.state.0.diligent_context.is_none() {
            self.state.0.diligent_context = self.render_device.diligent_context_handle();
        }
    }
    ...
    ///
    /// M1-4b-2: the encoder records on the Diligent immediate context (the
    /// raw-encoder render/compute passes and the uploads land there).
    ...
    ///
    /// M1-3: the pass records on the Diligent immediate context (a
    /// render-pass + framebuffer pair is created/reused through
    /// [`diligent_draw`](super::diligent_draw) and `BeginRenderPass` is
    /// issued); when the pass cannot begin (engine unavailable) an empty
    /// tracked pass is returned whose commands are all skipped (warned once
    /// per process - M1-4b-2: the wgpu fallback encoder is gone).
    pub fn begin_tracked_render_pass<'a>(
        ...
        // tracked pass (all commands skipped) - M1-4b-2: there is no wgpu
        // fallback encoder anymore.
        warn_on_empty_pass(&descriptor, err_reason);
        TrackedRenderPass::empty(&self.render_device)
    }
...
/// Warns (once per process) when a tracked render pass could not begin on
/// the diligent path; subsequent occurrences are logged at debug level.
fn warn_on_empty_pass(descriptor: &RenderPassDescriptor<'_>, reason: Option<String>) {
